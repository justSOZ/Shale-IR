...
if not os.path.exists(image_directory):
    ...
    for file in os.listdir(image_directory):
        if file.endswith('.jpg'):
            ...
            if len(parts) == 2 and parts[0].isdigit() and parts[1].endswith('.jpg') and parts[1][:-4].isdigit():
                col = int(parts[0])   Col = int（parts[0]）
                row = int(parts[1][:-4])
    ...
    if not images_files:
        ...
        with Image.open(first_image_path) as img:
            width, height = img.size   宽度，高度 = 图像的尺寸

        # 创建一个空白的大图来存放所有的图片
        merged_width = 70 * width合并宽度 = 70 * 宽度
        merged_height = 70 * height合并高度 = 70 * 高度

        # 打印合并后的大图像的尺寸
        print(f"Merged Image Dimensions: {merged_width}x{merged_height}")打印合并图像的尺寸：{merged_width}×{merged_height}
        ...
        # 保存合并后的图像为TIFF格式
        merged_image.save(os.path.join(image_directory, 'merged_image.tiff'), format='TIFF')合并后的图像保存为 TIFF 格式，路径为图像目录下的“merged_image.tiff”文件：
